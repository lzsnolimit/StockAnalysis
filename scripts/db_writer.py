...
def write_run_and_alerts(sqlite_path: str, analyzed: Dict[str, Any], run_ts: str | None = None, errors: List[str] | None = None) -> int:
    ...
    with _connect(sqlite_path) as conn:
        ...
        # insert run
        cur.execute(
            "INSERT INTO runs(run_ts, duration_ms, top10_count, alerts_count, errors_json) VALUES (?,?,?,?,?)",
            (run_ts, None, len(items), len(alerts), errors_json),
        )
        ...
        for item in items:
            ...
            cur.execute(
                """
                INSERT OR REPLACE INTO alerts(run_id, ticker, severity, attention_needed, reasons_json, email_subject, email_body, price, change_pct, news_count, data_snapshot_json, created_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (run_id, ticker, severity, attn, reasons_json, email_subject, email_body, price, change_pct, news_count, snapshot, run_ts),
            )
        conn.commit()
        return run_id
